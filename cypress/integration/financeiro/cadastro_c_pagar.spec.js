...
describe('Deve testar e validar o cadastro de contas a pagar', () => {

    const formas_pagto = ['Boleto', 'Outro', 'Nota Fiscal']
    const planos = ['Energia Elétrica', 'Fornecedores', 'Funcionários', 'Gás', 'Produtos de Limpeza', 'Rotação de Pasto', 'Operacional']
    ...
    beforeEach(() => {
        ...
        cy.xpath(cli_loc.MENU_LATERAL.CONTAS_PAGAR).click()
    })

    it('Cadastro de contas a pagar', function() {

        cy.get(cli_loc.FINANCEIRO.BTN_CADASTRAR_C_PAGAR).click()
        console.log(conta_pagar.vencimento)
        cy.get('input[formcontrolname="dueDate"]').type(conta_pagar.vencimento)
        cy.get('nz-form-label:contains("Favorecido") ~ nz-form-control').type(conta_pagar.favorecido)
        cy.get('nz-option-item:has(div:contains("' + conta_pagar.favorecido + '"))').click()
        cy.get('nz-form-label:contains("Forma de Pagamento") ~ nz-form-control').click()
